...
当 moviemaster agent 检测到 seedancer 仓库有任何代码更新（`git push`、新版本 tag、SKILL.md/references/ 文件变更）时，**必须自动执行以下完整流程**，全程无需人工干预：

### Step 1: 分析变更内容

扫描本次 `git diff`，识别哪些功能/模块/参考文档被新增、修改或删除。

### Step 2: 重写英文简介（README.md）

根据变更内容，重新设计与撰写完整的英文 README.md：
- 顶部语言切换栏必须保留：`<a href="README.md"><b>English</b></a> · <a href="docs/README-cn.md">中文</a>`
- 结构参考多语仓库最佳实践（README 为英文主文档，中文版放 `docs/README-cn.md`）
- 更新所有受变更影响的章节（版本历史、核心功能、模块描述、API 变更等）
- 保持专业、简洁、可读的风格

### Step 3: 重写中文简介（docs/README-cn.md）

根据英文版的最新内容，重新撰写完整的中文版 `docs/README-cn.md`：
- 顶部语言切换栏必须保留：`<a href="../README.md"><b>English</b></a> · <a href="README-cn.md">中文</a>`
- 翻译准确，专业术语保持一致
- 结构与英文版对齐
...
```bash
git add README.md docs/README-cn.md
git commit -m "docs: auto-sync README after repository update [skip ci]"
...
- **唯一分支**：`master`
- 禁止推送到 `main` 或其他分支
- 必须使用 `[skip ci]` 标记避免触发 CI
- `git config push.default current` 已配置

### 触发条件

- 任何 `git push` 到 seedancer 仓库后
- 任何新版本 tag 创建后
- SKILL.md 或 references/ 目录下任何文件被修改后